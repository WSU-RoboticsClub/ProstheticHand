...
class OpenBCIBoard(object):
  ...
  def print_incoming_text(self):
    ...
    if self.ser.inWaiting():
     ...
     #Look for end sequence $$$
      while '$$$' not in line:
        c = self.ser.read().decode('utf-8', errors='ignore') # This is necessary, it does not work with 'replace', it does work with 'ignore'
  ...
  def openbci_id(self, serial):
    ...
    if serial.inWaiting():
     ...
     #Look for end sequence $$$
      while '$$$' not in line:
        c = serial.read().decode('utf-8', errors='ignore') # we don't seem to be getting to this line
